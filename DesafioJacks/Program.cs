// See https://aka.ms/new-console-template for more informationusing RegraNegocio;using AcessoDados;using Segeus.Interface;using System.Text;using Sigeus.Model;using System.Collections.Generic;using System;var tela = new Tela();tela.MenuInicial();var comando = tela.LerComandoUsuario();var regraNegocio = new UsuarioRegraNegocio();while (comando != "9"){    switch (comando)    {        case "1"://listar usuario            tela.LimparTela();            var cabecalho = new StringBuilder();            cabecalho.Append("Código | Nome | Perfil | Data Cadastro | Tipo Acesso");            Console.WriteLine(cabecalho.ToString());            var listaUsuarios = regraNegocio.ListarUsuarios();            foreach (var user in listaUsuarios)            {                Console.WriteLine($"{user.IdUsuario}|{user.Usuario}|{user.Perfil}|{user.DataCadastro}|{user.IdTipoAcesso}");            }            Console.WriteLine("Pressione uma tecla para continuar...");            Console.ReadKey();            tela.MenuInicial();            comando = tela.LerComandoUsuario();            break;        case "2"://cadastrar usuario            try            {                tela.LimparTela();                var novoUsuario = new UsuarioModel()                {                    IdEmpresa = 1,//FabiNutri
        ...
        case "4"://Consultar Usuario
            ...
            if (usuarioConsultado != null)            {                cabecalho = new StringBuilder();                cabecalho.Append("Código | Nome | Perfil | Data Cadastro | Tipo Acesso");                Console.WriteLine(cabecalho.ToString());                Console.WriteLine($"{usuarioConsultado.IdUsuario}|{usuarioConsultado.Usuario}|{usuarioConsultado.Perfil}|{usuarioConsultado.DataCadastro}");
